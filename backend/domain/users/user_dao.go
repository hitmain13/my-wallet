...
import (
	"github.com/hitmain13/my-wallet/backend/datasource/mysql/users_db"
	"github.com/hitmain13/my-wallet/backend/utils/errors"
)

var (
	queryInsertUser     = "INSERT INTO users (first_name, last_name, email, password) VALUE (?, ?, ?, ?);"
	queryGetUserByEmail = "SELECT id, first_name, last_name, email, password FROM users WHERE email=?;"
	queryGetUserByID    = "SELECT id, first_name, last_name, password FROM users WHERE id=?;"
)
